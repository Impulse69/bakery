...
Schema lives in `apps/server/prisma/schema.prisma`. All Prisma commands must run from `apps/server/`.
...
npx prisma db seed          # Seed initial data (if seed script exists)
```
...
```
DATABASE_URL=postgresql://user:password@localhost:5432/bread_faculty?schema=public
JWT_SECRET=<secret>
PORT=3001                  # optional, defaults to 3001
```

The server reads `DIRECT_URL` first, then `DATABASE_URL` (see `prisma.config.ts`).
...
- **Login page**: Full-screen bakery background image with dark overlay + Bread Faculty logo above the title (`apps/desktop/src/renderer/assets/logo.png`, `login-bg.png`)
- **POS product cards**: Two-tier image system in `ProductGrid.tsx`. `PRODUCT_IMAGES` map keyed by exact product name takes priority; falls back to `CATEGORY_ICONS` (keyed by category), then `otherIcon`. Product images in `apps/desktop/src/renderer/assets/products/`, category icons in `assets/categories/`
...
- **apps/desktop**: Fully implemented. Electron main process, React Router shell, auth context, API/socket client libs. All 12 modules built: Dashboard, POS, Sales Orders, Products, Inventory, Inventory Counts, Production, Suppliers, Purchase Orders, Expenses, Customers, Reports. Only Settings remains a placeholder.
